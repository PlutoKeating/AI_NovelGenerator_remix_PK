...
export default function GlobalSummaryTab() {
  const novelPath = useNovelStore((s) => s.novelPath);
  const globalSummaryContent = useNovelStore((s) => s.globalSummaryContent);
  const loadFile = useNovelStore((s) => s.loadFile);
  const saveFile = useNovelStore((s) => s.saveFile);
  const [content, setContent] = React.useState(globalSummaryContent);
  const [loading, setLoading] = React.useState(false);

  React.useEffect(() => { setContent(globalSummaryContent); }, [globalSummaryContent]);

  const load = async () => {
    if (!novelPath) { toast({ title: "No novel path", variant: "destructive" }); return; }
    setLoading(true);
    const data = await loadFile("global_summary");
    setContent(data);
    setLoading(false);
  };
  ...
  const save = async () => {
    ...
    setLoading(true);
    try {
      await saveFile("global_summary", content);
      toast({ title: "Global Summary saved" });
    } catch (e: any) {
    ...
    <div className="flex flex-col h-full gap-2 p-2">
      <div className="flex items-center justify-between shrink-0">
        <div className="flex items-center gap-2">
          <Label className="shrink-0 font-semibold">Global Summary</Label>
